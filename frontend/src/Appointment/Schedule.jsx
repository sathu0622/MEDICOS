...
const Schedule = () => {
  ...
  useEffect(() => {
    if (user?.name) {
      setFormData(prev => ({ ...prev, doctor: user.name }));
    }

    const fetchSlots = async () => {
      try {
        setLoadingSlots(true);
        const res = await api.get('/ScheduleOperations/getslot');
        setSlots(res.data);
      } catch (err) {
        console.error('Error fetching slots:', err);
      } finally {
  ...
  const validateForm = () => {
    ...
    else if (formData.slotDate < today) newErrors.slotDate = "Date cannot be in the past.";
    ...
    if (formData.start && formData.end) {
      ...
      if (startTime >= endTime) newErrors.time = "Start time must be before end time.";
      else if ((endTime - startTime) / 60000 < 30)
        newErrors.time = "Time slot must be at least 30 minutes long.";
    }

    if (!formData.dContact.trim()) newErrors.dContact = "Contact is required.";
    else if (!/^\d{10}$/.test(formData.dContact)) newErrors.dContact = "Invalid contact number (10 digits required).";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.post('/ScheduleOperations/Schedule', {
        userId: user.id,
        ...formData
      });
      ...
      if (res.status === 201) {
        ...
        const updatedSlots = await api.get('/ScheduleOperations/getslot');
        setSlots(updatedSlots.data);

        // Reset only date/time/contact, keep doctor name
        setFormData(prev => ({
          ...prev,
          slotDate: '',
          start: '',
      ...
      <div className="container mx-auto px-4 py-8 mt-10">
        <div className="flex flex-col lg:flex-row justify-center items-start gap-8 w-full max-w-6xl mx-auto pt-16">
          ...
          <div className="bg-white p-8 rounded-xl shadow-md w-full lg:w-1/2 max-w-lg border border-gray-100">
            ...
            <form className="space-y-5" onSubmit={handleSubmit}>
              <input type="text" name="doctor" value={formData.doctor} readOnly
                     className="w-full p-3 border border-gray-300 rounded-lg bg-gray-100 text-black cursor-not-allowed" />

              <input type="date" name="slotDate" value={formData.slotDate} onChange={handleChange}
                     className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#007BFF]" />
              ...
              <input type="text" placeholder="Contact" name="dContact" value={formData.dContact} onChange={handleChange}
                     className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#007BFF]" />

              <div className="flex flex-col sm:flex-row justify-center gap-4 pt-2">
                <button type="submit" disabled={submitLoading}
                        className="w-full py-3 px-6 bg-[#007BFF] text-white rounded-lg hover:bg-[#0069d9]">
                  {submitLoading ? 'Processing...' : 'Confirm'}
                </button>
                <button type="button" onClick={() => navigate('/Profile')}
                        className="w-full py-3 px-6 bg-gray-200 rounded-lg hover:bg-gray-300">
                  Cancel
                </button>
              </div>
            </form>
          ...
          <div className="bg-white p-8 rounded-xl shadow-md w-full lg:w-1/2 max-w-lg border border-gray-100">
            <div className="flex flex-col items-center space-y-6">
              <img src={Doctorprofile} alt="Doctor" className="w-40 h-40 rounded-full border-4 border-[#007BFF]/20" />
              <div className="w-full space-y-4">
                <p><span className="font-semibold text-[#007BFF]">Name:</span> {formData.doctor}</p>
                <p><span className="font-semibold text-[#007BFF]">Date:</span> {formData.slotDate || 'Not specified'}</p>
                <p><span className="font-semibold text-[#007BFF]">Time:</span> {formData.start && formData.end ? `${formData.start} to ${formData.end}` : 'Not specified'}</p>
              </div>
            </div>
          </div>
          ...
          {loadingSlots ? <p>Loading slots...</p> :
            slots.length ? (
              <ul className="space-y-2">
                {slots.map(slot => (
                  <li key={slot._id} className="p-3 border rounded bg-gray-50">
                    {slot.slotDate} | {slot.start} - {slot.end} | Contact: {slot.dContact}
                  </li>
                ))}
              </ul>
