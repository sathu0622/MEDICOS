...
import { useNavigate, useParams } from "react-router-dom";
import api from "../services/api";
import { AuthContext } from "../context/AuthContext"; // Auth context to get user
...
const AppointmentBooking = () => {
  ...
  const validateForm = () => {
    ...
    if (!appointment.repname.trim() || appointment.repname.trim().length < 2) {
      errors.repname = "Representative name must be at least 2 characters";
      isValid = false;
    }
    if (!appointment.contact.trim() || !/^\S+@\S+\.\S+$/.test(appointment.contact)) {
      errors.contact = "Valid email is required";
      isValid = false;
    }
    if (!appointment.reason.trim() || appointment.reason.trim().length < 10) {
      errors.reason = "Description must be at least 10 characters";
      isValid = false;
    }
    if (!appointment.address.trim() || appointment.address.trim().length < 10) {
      errors.address = "Address must be at least 10 characters";
      isValid = false;
    }
    if (!appointment.company.trim() || appointment.company.trim().length < 2) {
      errors.company = "Company name must be at least 2 characters";
      isValid = false;
    }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const bookingData = {
        userId: user._id,  // obtained from AuthContext
        slotId: id,
        repname: appointment.repname.trim(),
        contact: appointment.contact.trim(),
        address: appointment.address.trim(),
  ...
  if (loading || userLoading) return <div className="text-center mt-32">Loading slot details...</div>;
  if (error) return <div className="text-center mt-32 text-red-500">{error}</div>;
  if (!slot) return <div className="text-center mt-32 text-red-500">Slot not found</div>;
